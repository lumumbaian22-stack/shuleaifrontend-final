...
class ParentDashboard extends window.BaseDashboard {
        ...
        this.container.innerHTML = `
            <div class="space-y-6 animate-fade-in">
                <div class="rounded-xl border bg-card p-6 bg-gradient-to-r from-amber-50 to-orange-50">
                    ...
                    <p class="text-muted-foreground mt-2">Welcome, ${this.escapeHtml(user.name) || 'Parent'}!</p>
                </div>
                
                ${this.children.length > 0 ? `
                <div class="flex gap-2 border-b pb-4 overflow-x-auto">
                    ${this.children.map(child => `<button class="px-4 py-2 bg-muted rounded-lg">${child.User?.name || 'Child'} (Grade ${child.grade})</button>`).join('')}
                </div>
