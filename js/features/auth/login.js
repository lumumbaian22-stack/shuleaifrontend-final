    ...
    async signIn(role) {
        ...
        try {
            ...
            if (response.success) {
                ...
                if (response.data.school) localStorage.setItem('school', JSON.stringify(response.data.school));
                
                toast.success('Login successful');
                modalManager.close('auth-modal');
                
                // Switch UI to dashboard container
                const landingPage = document.getElementById('landing-page');
                const dashboardContainer = document.getElementById('dashboard-container');
                if (landingPage) landingPage.style.display = 'none';
                if (dashboardContainer) dashboardContainer.style.display = 'block';
                
                // Load dashboard WITHOUT reloading the page
                if (window.loadDashboard) {
                    await window.loadDashboard(user.role);
                } else {
                    console.error('loadDashboard not available');
                    window.location.reload(); // fallback (should not happen)
                }
            }
        } catch (error) {
